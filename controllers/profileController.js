...
async function updateProfile(req, res) {
  ...
  try {
    // Ambil data pengguna saat ini menggunakan model
    ...
    if (password && await userModel.verifyPassword(password, currentUser.password)) {
    ...
    res.status(400).json({ error: true, message: error.message || 'Failed to update profile' });
  }
}
